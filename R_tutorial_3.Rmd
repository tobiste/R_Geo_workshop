...
data <- read_geochron("Data/Geochron_sample_download.xls")
...
```{r}
sample1$st.Pb206U238.perc <- sample1$st.Pb206U238/sample1$t.Pb206U238
```
...
ggplot(data = sample1, aes(x = Sample_ID, y = st.Pb206U238.perc)) +
  geom_boxplot() +
  labs(title = "Boxplot", x = NULL, y = '206/238 Age uncertainties (%)' ) 
```
...
ggplot(data = sample1, aes(x = st.Pb206U238.perc)) +
  geom_histogram(color='white') +  # adds the hisogram
