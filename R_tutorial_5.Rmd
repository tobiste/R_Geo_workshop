...
map2
```
Th eplot above will produce some default outputs (colors). 
...
map2.2
```
We do not have to define the color platte. `ggplot()` offers a vafiety of pre-installed color palettes.
...
We can also create spatial objects, e.g. a line, by giving the coordinates and the CRS:
...
dem.cropped <- raster::crop(dem, crop_extent)
plot(dem.cropped)
...
Since we tranformed the raster dataset, the pixel cells are not equally spaced anymore,. Thus, we use `geom_tile()` that allows distorted pixels:
...
`swathR` calculates the elevation data in our swath. To define the swath we have to define `k` -- the number of lines parallel to our profile line ("line.pts"), `dist` -- the distance of the lines to our profile line, and the cordinate reference system -- `crs`.
...
```{r extract_swath, warning=FALSE, message=FALSE}
...
You can use my function `orthodrome` (= great circle distance) to get the distance between two locations on earth:
```{r orthodrome}
distance.km <- orthodrome(line.pts[1, ], line.pts[2, ])
...
idw.res <- gstat::idw(formula = mag ~ 1, newdata = grid, locations = earthquakes.geo) %>%
  as.data.frame()
names(idw.res)[1:3] <- c("x", "y", "var.pred")
```

Now we have our IDW output we can plot it using ggplot2. 
```{r idw.map, message=FALSE, warning=FALSE}
map3 <- ggplot() +

  # add the raster with coloring (CPU intensive!!!)
  geom_tile(data = idw.res, aes(x, y, fill = var.pred)) +
  geom_path(
    data = plates.sp,
    aes(long, lat, group = group),
    lwd = 2,
