...
![Figure 1: Angles expressed by vector components](C:/Users/tobis/Documents/GIT_repos/R_Geo_workshop/Orientation_analysis/mean_dir1.png)
...
![Figure 2: Mean resultant vector](C:/Users/tobis/Documents/GIT_repos/R_Geo_workshop/Orientation_analysis/mean_dir2.png)
...
The parameter $\kappa$ is difficult to measure directly, but the concentration of the sample can be estimated from $\bar{R}$ (MLE). `tectonicr:::est.kappa()` estimates the parameter $\hat{kappa$} from the sample:
...
## Test for uniformity
The so-called Rayleigh Test checks if the data is randomly distributed or not. The Null Hypothesis is

$H_0$: $p = 0$ (i.e. the population is uniformly (randomly) distributed around the circle)

When $H_0$ is rejected, the alternative hypothesis is

$H_A$: $p \neq 0$ (i.e. the population is not uniformly (randomly) distributed around the circle. Thus, there is a preferred orientation.)
...
$\hat{\kappa}$ is the maximum likelihood estimate of $\kappa$. 
$z_alpha$ denotes the upper quantile of linear normal distribution. This is appropriate for $\hat{\kappa} \geq 2$ or $n\geq 10$ (Mardia and Jupp, 20000.)
...
## Exercises
### Bird homing 
In a pidgeon-homing experiment by Schmidt-Koening (1963), the vanishing angles of 15 birds were:
```{r pidgeon, echo=TRUE}
...
- Homing direction was 149$^{\circ}$. Is there a preferred homing direction (at 5% significance level)?
...
- Mean orientation?
- Normally distributed?
- A todays location of a glacier is at  130$^{\circ}$ compass bearing. Does the stria origin from this glacier?
