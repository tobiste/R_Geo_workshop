...
Now the data is georeferenced! To plot the dat awith `ggplot()` we use ` geom_sf()` and plot extents can be changed with `coord_sf()`:
...
```
R uses the *offical* projection/transformation parameters, either as EPSG, PROJ4 or WKT notation (they are used by ArcGIS or QGIS as well). A list of these CRS parameters can be found at: https://proj.org/en/9.3/operations/index.html
